...
# Рассчитываем МЖЗ, ГЗ, ПУ и qо для обеих систем
...
df_with_names.loc[condition_fixed_interval, 'Оптимальный размер заказа (qо)'] = (
    df_with_names.loc[condition_fixed_interval, 'Количество'] / 252).astype('float64')
...
with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
    ...
    system_column_letter = worksheet.cell(row=1,
                                          column=worksheet.max_column).column_letter  # Последний столбец - "СИСТЕМА УПРАВЛЕНИЯ ЗАПАСАМИ"
