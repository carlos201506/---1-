...
df_aggregated.rename(columns={'Unnamed: 2': 'Единицы измерения',
                              ...
                              'Unnamed: 5': 'Сумма без НДС'}, inplace=True)
...
df_with_names.loc[condition_fixed_interval, 'Максимальный желательный уровень запасов (МЖЗ)'] = (
    (df_with_names['Количество'] / 252) * (5 + 1)).astype('float64')
# Расчет ГЗ
df_with_names.loc[condition_fixed_interval, 'Гарантийный уровень запасов (ГЗ)'] = (
    (df_with_names['Количество'] / 252) * 5).astype('float64')

# Пороговый уровень запасов (ПУ)
df_with_names.loc[condition_fixed_interval, 'Пороговый уровень запасов (ПУ)'] = (
    df_with_names.loc[condition_fixed_interval, 'Максимальный желательный уровень запасов (МЖЗ)'] -
    (df_with_names['Количество'] / 252) * 1).astype('float64')

# Оптимальный размер заказа (qо)
df_with_names.loc[condition_fixed_interval, 'Оптимальный размер заказа (qо)'] = np.sqrt(
    (2 * df_with_names.loc[condition_fixed_interval, 'Количество']) / 252
).astype('float64')

#Код для заполнения "Интервал времени между заказами (I)"
#condition_fixed_interval = df_with_names["СИСТЕМА УПРАВЛЕНИЯ ЗАПАСАМИ"] == "Система с фиксированным интервалом времени между заказами"

#df_with_names.loc[condition_fixed_interval, 'Интервал времени между заказами (I)'] = (
#   (252 * df_with_names.loc[condition_fixed_interval, 'Оптимальный размер заказа (qо)']) / df_with_names['Количество']).astype('float64')
#df_with_names['Интервал времени между заказами (I)'] = 0.0

df_with_names.loc[condition_fixed_interval, 'Интервал времени между заказами (I)'] = (252 * (df_with_names.loc[condition_fixed_interval, 'Оптимальный размер заказа (qо)'])
                                                                                     / (df_with_names.loc[condition_fixed_interval,'Количество'])).astype('float64')
...
df_with_names.loc[condition_fixed_order_size, 'Максимальный желательный уровень запасов (МЖЗ)'] = (
    (df_with_names['Количество'] / 252) * (5 + 1)).astype('float64')
...
# Оптимальный размер заказа (qо)
df_with_names.loc[condition_fixed_order_size, 'Оптимальный размер заказа (qо)'] = (
    df_with_names.loc[condition_fixed_order_size, 'Количество'] / 252).astype('float64')

df_with_names.loc[condition_fixed_order_size, 'Интервал времени между заказами (I)'] = (252 * (df_with_names.loc[condition_fixed_order_size, 'Оптимальный размер заказа (qо)'])
                                                                                     /(df_with_names.loc[condition_fixed_order_size,'Количество'])).astype('float64')
...
# Расчеты для строк с установленной периодичностью пополнения запасов
df_with_names.loc[condition_fixed_replenishment_periodicity, 'Максимальный желательный уровень запасов (МЖЗ)'] = (
    (df_with_names['Количество'] / 252) * (5 + 1)).astype('float64')

df_with_names.loc[condition_fixed_replenishment_periodicity, 'Гарантийный уровень запасов (ГЗ)'] = (
    (df_with_names['Количество'] / 252) * 5).astype('float64')

# Пороговый уровень запасов (ПУ)
df_with_names.loc[condition_fixed_replenishment_periodicity, 'Пороговый уровень запасов (ПУ)'] = (
    df_with_names.loc[condition_fixed_replenishment_periodicity, 'Максимальный желательный уровень запасов (МЖЗ)'] -
    (df_with_names['Количество'] / 252) * 1).astype('float64')

# Оптимальный размер заказа (qо)
df_with_names.loc[condition_fixed_replenishment_periodicity, 'Оптимальный размер заказа (qо)'] =  np.sqrt(
    (2 * df_with_names.loc[condition_fixed_replenishment_periodicity, 'Количество']) / 252

).astype('float64')

# Интервал времени между заказами (I)

df_with_names.loc[condition_fixed_replenishment_periodicity, 'Интервал времени между заказами (I)'] = (
    (252 * df_with_names.loc[condition_fixed_replenishment_periodicity, 'Оптимальный размер заказа (qо)']) / df_with_names['Количество']).astype('float64')
...
output_file_path = r'C:/Users/Misha/Desktop/Pandas/main_AX_AY_AZ_BX_BY_BZ_CX_CY_CZ_analysis_results3.xlsx'
